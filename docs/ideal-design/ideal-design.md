# Ideal Folder Structure for a Skills + Agents Project

This document has been split into per-section files so they can be edited in parallel. Each link below points to the standalone section file.

---

## 1. Skill Anatomy

**Status: finalized.** See [`01-skill-anatomy.md`](01-skill-anatomy.md).

---

## 2. Agent Anatomy

See [`02-agent-anatomy.md`](02-agent-anatomy.md).

---

## 3. Shared Fragments

See [`03-shared-fragments.md`](03-shared-fragments.md).

---

## 4. Cross-Platform vs Claude-Only

See [`04-cross-platform-vs-claude-only.md`](04-cross-platform-vs-claude-only.md).

---

## 5. Agent-Orchestrates-Agent

See [`05-agent-orchestrates-agent.md`](05-agent-orchestrates-agent.md).

---

## 6. Python in Skills

See [`06-python-in-skills.md`](06-python-in-skills.md).

---

## 7. Top-Level Layout

See [`07-top-level-layout.md`](07-top-level-layout.md). *Deferred — synthesizes chapters 3–6; revisit after they are finalized.*

---

## 8. Worked Example

See [`08-worked-example.md`](08-worked-example.md).
